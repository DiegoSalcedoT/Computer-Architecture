...
class UltraSuperCalculator:
  ...
  def substract(self, address_num1, address_num2):
    num1 = self.load_value_from_register(address_num1)
    num2 = self.load_value_from_register(address_num2)
    return num1 - num2
  ...
  def get_last_calculation(self):
    if self.temp_history_index - 1 < 0:
      self.temp_history_index = 9
    else:
      self.temp_history_index -= 1
    last_value = f"Last result was: {int(self.history_registers[self.temp_history_index], 2)}"
  ...
  def binary_reader(self, instruction):
    ...
    if function_code == '100000':
      ...
      result = self.substract(source_one, source_two)
...
FirstCalculator = UltraSuperCalculator('Larry')

# Adds 5 and 10 to number registers
FirstCalculator.binary_reader("00000100000000000000000101000000")
FirstCalculator.binary_reader("00000100000000000000001010000000")

# Adds/Subtracts/Multiplies/Divides 5 and 10 from registers
FirstCalculator.binary_reader("00000000001000100000000000100000")
FirstCalculator.binary_reader("00000000001000100000000000100010")
FirstCalculator.binary_reader("00000000001000100000000000011000")
FirstCalculator.binary_reader("00000000010000010000000000011010")

# Gets the last three calculations
FirstCalculator.binary_reader("10000100000000000000000000000000")
FirstCalculator.binary_reader("10000100000000000000000000000000")
FirstCalculator.binary_reader("10000100000000000000000000000000")
FirstCalculator.binary_reader("10000100000000000000000000000000")
FirstCalculator.binary_reader("10000100000000000000000000000000")
